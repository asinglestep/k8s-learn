...
### (4). 将token.csv文件中的kubelet-bootstrap 用户赋予 system:node-bootstrapper cluster 角色(role)，kubelet 才能有权限创建认证请求
```
$ cd /etc/k8s
...
修改
ETCD_ENDPOINTS="http://172.16.221.131:2379"
FLANNEL_ETCD_PREFIX="/kube-centos/network"

(2). 在etcd中创建网络配置
etcdctl set /kube-centos/network/config '{ "Network" : "10.1.0.0/16" }'

(3). 创建/run/flannel/subnet.env和/run/docker_opts.env文件，如果存在跳过此步骤
写入
FLANNEL_NETWORK=10.1.0.0/16
FLANNEL_SUBNET=10.1.49.1/24
FLANNEL_MTU=1472
FLANNEL_IPMASQ=false
内容到/run/flannel/subnet.env文件中

去github.com/coreos/flannel下载mk-docker-opts.sh文件
sudo ./mk-docker-opts.sh -i
生成/run/docker_opts.env
...
(2). 创建/run/flannel/subnet.env和/run/docker_opts.env文件，如果存在跳过此步骤
写入
FLANNEL_NETWORK=10.1.0.0/16
FLANNEL_SUBNET=10.1.50.1/24
FLANNEL_MTU=1472
FLANNEL_IPMASQ=false
内容到/run/flannel/subnet.env文件中
...
example-service   NodePort    10.254.109.88   <none>        80:31183/TCP   3h
